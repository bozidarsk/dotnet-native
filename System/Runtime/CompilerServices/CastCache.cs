...
namespace System.Runtime.CompilerServices
{
    ...
    internal unsafe struct CastCache
    {
        ...
        private int[]? CreateCastCache(int size, bool throwOnFail = false)
        {
            ...
            byte shift = (byte)BitOperations.LeadingZeroCount((uint)(size - 1));
            ...
            do
            {
                ...
                for (int i = 0; i < BUCKET_SIZE;)
                {
                    ...
                    if (version == 0 || (version >> VERSION_NUM_SIZE) > i)
                    {
                        ...
                        uint versionOrig = (uint)Interlocked.CompareExchange(ref Unsafe.As<uint, int>(ref pEntry._version), (int)newVersion, (int)version);
